...
class HomeScreen extends StatelessWidget {
  ...
  Widget _hero() => Padding(
    ...
    child: Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(17)), child: const TextField(decoration: InputDecoration(hintText: 'What are you looking for?', prefixIcon: Icon(Icons.search_rounded, color: purple), suffixIcon: Icon(Icons.tune_rounded, color: muted), border: InputBorder.none, contentPadding: EdgeInsets.symmetric(vertical: 16)))),
  ...
  Widget _studentCard(BuildContext context) => Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: Container(padding: const EdgeInsets.all(19), decoration: BoxDecoration(color: const Color(0xFF17172B), borderRadius: BorderRadius.circular(24)), child: Row(children: [
    ...
    const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('New to CampusSupply?', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w800)), SizedBox(height: 4), Text('Sign in or create an account to save favourites and personalize your experience.', style: TextStyle(color: Color(0xFFB4B1C5), fontSize: 11, height: 1.35))])),
    const SizedBox(width: 8),
    IconButton(onPressed: () => Navigator.pushNamed(context, '/login'), icon: const Icon(Icons.arrow_forward_rounded, color: Colors.white)),
  ]));
