...
class HomeScreen extends StatelessWidget {
            ...
            Container(
              ...
              child: const TextField(
                decoration: InputDecoration(
                  ...
                  prefixIcon: Icon(Icons.search_rounded, color: purple),
  ...
  Widget _chip(IconData icon, String text) {
    return Expanded(
      child: Container(
        ...
        child: Row(children: [Icon(icon, color: Colors.white, size: 16), const SizedBox(width: 6), Flexible(child: Text(text, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 10.5, fontWeight: FontWeight.w700)))]),
      ),
    );
  }
        ...
        SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (_, i) => Container(
              width: 94,
              decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: const Color(0xFFEAE8F2))),
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Container(width: 44, height: 44, decoration: BoxDecoration(color: const Color(0xFFF0EDFF), borderRadius: BorderRadius.circular(14)), child: Icon(data[i].$1, color: purple, size: 22)),
                  const SizedBox(height: 8),
                  Text(data[i].$2, style: const TextStyle(color: ink, fontWeight: FontWeight.w700, fontSize: 11)),
                ],
              ),
            ),
          ),
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ListView(
            ...
            padding: const EdgeInsets.symmetric(horizontal: 20),
            children: [
              _card(Icons.menu_book_rounded, 'Study essentials', 'Notes, books & guides', 'BESTSELLER'),
              const SizedBox(width: 14),
              _card(Icons.draw_rounded, 'Creative kit', 'Pens, markers & art', 'STUDENT PICK'),
              const SizedBox(width: 14),
  ...
  Widget _card(IconData icon, String title, String subtitle, String tag) {
    return SizedBox(
      width: 190,
      child: Container(
        padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(22), border: Border.all(color: const Color(0xFFEAE8F2))),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Expanded(
              child: Container(
                ...
                decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFFF2F0FF), Color(0xFFEAE5FF)]), borderRadius: BorderRadius.circular(17)),
                child: Stack(
                  children: [
                    Center(child: Icon(icon, size: 58, color: purple)),
                    Positioned(
                      top: 9,
                      left: 9,
                      child: Container(
                        ...
                        child: Text(tag, style: const TextStyle(fontSize: 8, fontWeight: FontWeight.w800, color: purple)),
                      ),
                    ),
                  ],
                ),
            ...
            Text(title, style: const TextStyle(color: ink, fontSize: 14, fontWeight: FontWeight.w800)),
  ...
  Widget _studentCard() {
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: const Color(0xFF17172B), borderRadius: BorderRadius.circular(24)),
        child: Row(
          children: [
            Container(
              width: 48,
              height: 48,
              decoration: BoxDecoration(color: const Color(0xFF2A2844), borderRadius: BorderRadius.circular(15)),
              child: const Icon(Icons.auto_awesome_rounded, color: Color(0xFFC9BDFF)),
            ),
            ...
            const Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Make it yours.', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w800)),
                  SizedBox(height: 4),
                  Text('Sign in to save favourites and get a more personal campus experience.', style: TextStyle(color: Color(0xFFB4B1C5), fontSize: 11, height: 1.35)),
                ],
              ),
            ),
            const SizedBox(width: 8),
  ...
  Widget _title(String title, String action) {
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(child: Text(title, style: const TextStyle(color: ink, fontSize: 19, fontWeight: FontWeight.w800, letterSpacing: -.4))),
          Text(action, style: const TextStyle(color: purple, fontSize: 11.5, fontWeight: FontWeight.w800)),
        ],
      ),
    );
  }
  ...
  Widget _roundIcon(IconData icon) {
    return Container(
      width: 42,
      height: 42,
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), border: Border.all(color: const Color(0xFFEAE8F2))),
